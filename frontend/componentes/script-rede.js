/* //..........ROLAGEM INFINITA..........
// Função para adicionar conteúdo dinâmico à div scrollable
function addDynamicContent() {
    var contentContainerRede = document.getElementById('contentContainerRede');
    var divPostagem = document.createElement('div');
    divPostagem.className = 'principal postagens';

    var content = `
        <div class="image-with-text">
            <img src="../imagens/perfis/foto.png" alt="Imagem" height="30">
            <b><em class="titulo-postagem">Fulano de Tal</em></b>
        </div>
        <div class="date-time-location">15 / jan / 2024 &nbsp;&nbsp;&nbsp;10:00 am&nbsp;&nbsp;&nbsp;Mauá - SP</div>
        <img src="../imagens/rede/jogoteste.jpg" style="height: 280px;">
        <div><p><strong>Everdell</strong></p></div>
        <div><p>Hoje foi dia de conhecer a expansão de Everdell</p></div>
        <div class="likes">
            <a href="#"><img src="../imagens/icones/like.png" height="20" alt="Ícone 1"></a>
            <a href="#"><img src="../imagens/icones/dislike.png" height="20" alt="Ícone 2"></a>
            123456 (nº like - dislike)
        </div>
    `;

    divPostagem.innerHTML = content;
    contentContainerRede.appendChild(divPostagem);
}

// Função para replicar o conteúdo continuamente quando o mouse é movido sobre a área desejada
function replicateContentOnMouseMove() {
    var contentContainerRede = document.getElementById('contentContainerRede');
    contentContainerRede.addEventListener('mousemove', function() {
        addDynamicContent();
    });
}

// Chamar a função de replicar o conteúdo continuamente
replicateContentOnMouseMove();

// Selecionar o elemento de entrada de faixa e a div de valor
var barraDificuldade = document.getElementById('barraDificuldade');
var valorDificuldade = document.getElementById('valorDificuldade');

// Adicionar um ouvinte de evento de mudança à barra de dificuldade
barraDificuldade.addEventListener('input', () => {
    // Atualizar o conteúdo da div com o valor atual da faixa
    valorDificuldade.textContent = barraDificuldade.value;
}); */









// Função para formatar a data no formato desejado (dd / mmm / aaaa &nbsp;&nbsp;&nbsp;hh:mm am/pm)
function formatarData(data) {
    const dataObj = new Date(data);
    const dia = dataObj.getDate();
    const mes = dataObj.toLocaleDateString('pt-BR', { month: 'short' });
    const ano = dataObj.getFullYear();
    const hora = dataObj.toLocaleTimeString('pt-BR', { hour: 'numeric', minute: 'numeric' });

    return `${dia} / ${mes} / ${ano} &nbsp;&nbsp;&nbsp;${hora}`;
}

// Função para formatar a duração da partida no formato hh"h" mm"min" ou ss"s"
function formatarDuracao(duracao) {
    const duracaoSegundos = parseInt(duracao.split(' ')[0], 10);

    if (duracaoSegundos >= 3600) {
        const horas = Math.floor(duracaoSegundos / 3600);
        const minutos = Math.floor((duracaoSegundos % 3600) / 60);
        return `${horas}h ${minutos}min`;
    } else if (duracaoSegundos >= 60) {
        const minutos = Math.floor(duracaoSegundos / 60);
        return `${minutos}min`;
    } else {
        return `${duracaoSegundos}s`;
    }
}

// Função para adicionar conteúdo dinâmico à div scrollable
async function addDynamicContent() {
    var contentContainerRede = document.getElementById('contentContainerRede');

    try {
        const response = await fetch('https://api-noob.onrender.com/api/atividades');
        const atividades = await response.json();
        const randomAtividade = atividades[Math.floor(Math.random() * atividades.length)];

        var divPostagem = document.createElement('div');
        divPostagem.className = 'principal postagens';

        var fotoUsuario = randomAtividade.usuarios[0].src;
        var nomeUsuario = randomAtividade.usuarios[0].nome;

        var fotoVencedor = randomAtividade.vencedor[0] ? randomAtividade.vencedor[0].src : '';
        var pontuacaoVencedor = randomAtividade.vencedor[0] && randomAtividade.vencedor[0].pontuacao ? ` (${randomAtividade.vencedor[0].pontuacao})` : '';
        var duracaoPartida = randomAtividade.vencedor[0] ? formatarDuracao(randomAtividade.vencedor[0].duracao) : '';

        var tabelaParticipantes = '<table>';
        randomAtividade.usuarios.forEach(usuario => {
            tabelaParticipantes += `<tr><td>${usuario.nome}</td>${usuario.pontuacao ? `<td>${usuario.pontuacao}</td>` : ''}</tr>`;
        });
        tabelaParticipantes += '</table>';

        var content = `
            <div class="image-with-text">
                <img src="${fotoUsuario}" alt="Imagem" height="30">
                <b><em class="titulo-postagem">${nomeUsuario}</em></b>
            </div>
            <div class="date-time-location">${formatarData(randomAtividade.createdAt)}</div>
            <img src="${fotoVencedor}" style="height: 280px;">
            <div><p><strong>${randomAtividade.jogo[0].titulo}</strong></p></div>
            <div class="likes">
                <a href="#"><img src="../imagens/icones/like.png" height="20" alt="Ícone 1"></a>
                <a href="#"><img src="../imagens/icones/dislike.png" height="20" alt="Ícone 2"></a>
                123456 <!-- (nº like - dislike) (valor ficticio aleatório, por enquanto) -->
            </div>
            <div>
                <p>${duracaoPartida}</p>
                ${tabelaParticipantes}
            </div>
        `;

        divPostagem.innerHTML = content;
        contentContainerRede.appendChild(divPostagem);

    } catch (error) {
        console.error('Erro ao buscar as atividades:', error);
    }
}

// Função para carregar mais conteúdo quando a rolagem atinge o final
function handleScroll() {
    var contentContainerRede = document.getElementById('contentContainerRede');
    var scrollPosition = window.scrollY + window.innerHeight;
    var threshold = contentContainerRede.scrollHeight - 100;

    if (scrollPosition >= threshold) {
        addDynamicContent();
    }
}

// Adicionar ouvinte de evento de rolagem
window.addEventListener('scroll', handleScroll);

// Carregar conteúdo inicial
addDynamicContent();
